...
Metaphone3* Metaphone_new(const char* wordtobemeta){
    return new Metaphone3(wordtobemeta);
}

void setEncodeVowels(Metaphone3* meta,BOOL val){
    meta->SetEncodeVowels(val);
}

void setEncodeExact(Metaphone3* meta,BOOL val){
    meta->SetEncodeExact(val);
}

void encode(Metaphone3* meta){
    meta->Encode();
}

const char* getMetaph(Metaphone3* meta){
    return meta->GetMetaph();
}

const char* altGetMetaph(Metaphone3* meta){
    return meta->GetAlternateMetaph();
}
...
void Metaphone3::ConvertExtendedASCIIChars()
{
	...
	{
        wchar_t c = m_inWord[i];
		if(c >= L'à' && c <= L'þ')
		{
			m_inWord[i] = (L'À' + c - L'à');
		}
		if(c >= L'' && c <= L'')
		{
			m_inWord[i] = (L'' + c - L'');
